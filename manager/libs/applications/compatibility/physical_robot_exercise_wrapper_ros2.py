...
class CompatibilityExerciseWrapperRos2(IRoboticsPythonApplication):
    def __init__(self, exercise_command, gui_command, update_callback):
        ...
        process_ready, self.exercise_server = self._run_exercise_server(
            f"python3 {exercise_command}",
        ...
        process_ready, self.gui_server = self._run_exercise_server(
            f"python3 {gui_command}", f"{home_dir}/ws_gui.log", "websocket_gui=ready"
        )
        if process_ready:
    ...
    def send_freq(self, exercise_connection, is_alive):
        ...
        while is_alive():
            exercise_connection.send("""#freq{"brain": 20, "gui": 10, "rtf": 100}""")
            time.sleep(1)

    def start_send_freq_thread(self):
        """Start a thread to send the frequency of the brain and gui to the exercise server"""
        daemon = Thread(
            target=lambda: self.send_freq(
                self.exercise_connection, lambda: self.is_alive
            ),
            daemon=False,
    ...
    def server_message(self, name, message):
        if name == "gui":  # message received from GUI server
            ...
            LogManager.logger.info(f"Message received from exercise: {message[:40]}")
