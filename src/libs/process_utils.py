...
def stop_process_and_children(process: Popen, signal: int = 9, timeout: int = None):
    ...
    try:
        ...
        # TODO: log
        # LogManager.logger.info(f"Process {name} and it's children were stopped")
        print(f"Process {name} and it's children were stopped")

        return gone, alive
    except psutil.NoSuchProcess:
        # TODO: log
        # LogManager.logger.info(f"Process with pid {process.pid} is already closed")
        print(f"Process with pid {process.pid} is already closed")
